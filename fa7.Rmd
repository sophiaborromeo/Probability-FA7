...
For this assessment, the group identified time between students arriving
at an elevator- a practical campus problem where events occur randomly
over time.
...
Because the group recorded interarrival times, this implies independence
— since each record is a time interval not influenced by the last one.
Since students come from various locations at random, this also
indicates random occurrence.So, it's safe to assume events are
independent and random over time. Therefore, we can apply the
...
the sum of all the waiting times. Through manual calculation, we get:
...
cat("Probability a student arrives within 30 seconds:", round(prob_within_30 * 100, 2), "%\n")
```


**b. Probability of waiting more than 60 seconds:** 
$$
P(X > x) = e^{-\lambda x}
$$
...
cat("Probability of waiting more than 60 seconds:", round(prob_more_than_60 * 100, 2), "%\n")
```

### Implications

**a. Expected Waiting Time:** Based on the data, the mean waiting time
between student arrivals is approximately:

```{r}
mean_wait <- 1 / lambda
cat("Expected (mean) waiting time:", round(mean_wait, 2), "seconds\n")
```

On average, a student arrives every 54.7 seconds.
...
ggplot(exp_data, aes(x = Time)) +
  ...
  geom_line(aes(y = CDF, color = "CDF"), size = 1.2, linetype = "dashed") +
